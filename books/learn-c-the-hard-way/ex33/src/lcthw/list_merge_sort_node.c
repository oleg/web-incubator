...
List *List_split_node(List *left) {
...
List *List_merge_node(List *left, List *right, List_compare cmp) {
...
List *List_merge_sort_node(List *left, List_compare cmp)
{
  if (left->count == 1) {
    return left;
  }
  List *right = List_split_node(left);//funny
  
  List *left_sorted = List_merge_sort_node(left, cmp);
  List *right_sorted = List_merge_sort_node(right, cmp);
  
  return List_merge_node(left_sorted, right_sorted, cmp);
}
