...
class IffTest < Test::Unit::TestCase

  def test_to_s
    iff = Iff.new(LessThan.new(Number.new(10), Number.new(11)),
  ...
  def test_reduce_true
    st = Iff.new(LessThan.new(Number.new(10), Number.new(11)),
                  Assign.new(:x, Number.new(100)),
                  Assign.new(:y, Number.new(200)))
    env = {}
    ...
    assert_equal Iff.new(Boolean.new(true),
                         Assign.new(:x, Number.new(100)),
  ...
  def test_reduce_false
    st = Iff.new(LessThan.new(Number.new(11), Number.new(10)),
                  Assign.new(:x, Number.new(100)),
                  Assign.new(:y, Number.new(200)))
    env = {}
    ...
    st_1, env_1 = st.reduce(env)
    assert_equal Hash[], env_1
    assert_equal Iff.new(Boolean.new(false),
                         Assign.new(:x, Number.new(100)),
                         Assign.new(:y, Number.new(200))), st_1
